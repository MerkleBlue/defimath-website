...
export const metadata: Metadata = {
    title: "Solidity Log2 Function - 406 Gas Fixed-Point - DeFiMath Docs",
    description: "Solidity base-2 logarithm in 18-decimal fixed-point — 406 gas, 1.6e-15 max rel. / 1.0e-15 max abs. error. Change-of-base log₂(x) = ln(x) / ln(2) on DeFiMath's 390-gas ln.",
...
export default function Page() {
    return (
        <FunctionDetail
            ...
            module="Math"
            name="log2"
            summary="Computes the base-2 logarithm of x in 18-decimal fixed-point."
            gas="406"
            absError="1.0e-15"
            ...
            howItWorks={(
                <>
                    ...
                    <p>
                        Because the implementation is a thin wrapper over <Link href="/docs/math/ln/" className="text-primary underline">ln</Link>, every guarantee <code className="text-primary">ln</code> provides — domain (<code className="text-primary">x &gt; 0</code>), precision, sign behavior — flows through directly. The only added error term is the rounding of <code className="text-primary">ln(2)</code> to 18 decimals, which is dwarfed by <code className="text-primary">ln</code>&apos;s own ~2e-15 error.
                    </p>
                </>
            )}
            limits={{
...
int256  l2 = Math.log2(x);      // l2 ≈ 3e18  (= log₂(8))

uint256 y  = 0.5e18;                // y = 0.5
int256  ly = Math.log2(y);      // ly ≈ −1e18 (= log₂(0.5))`}
