...
coverImage: /images/blog/clz-opcode-solidity-cover.webp
---
...
The savings don't stop at the primitives. `sqrt` and `ln` are the building blocks of Black-Scholes option pricing: both `callOptionPrice` and `putOptionPrice` call `ln(spot / strike)` once and a sqrt-based time-adjusted volatility once per evaluation. Swapping in the CLZ-powered versions of those two primitives drops [`callOptionPrice`](/docs/black-scholes/) from ~3,100 gas to **2,876 gas** — roughly 10% cheaper, with zero change to the option-pricing math itself.
